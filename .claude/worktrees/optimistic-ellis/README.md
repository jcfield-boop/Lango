...
Navigate to `http://langoustine.local` (or `http://192.168.0.44`).
...
```
[Browser WS]───────▶ ws_server.c ──▶ message_bus (inbound)
...
[Cron / Heartbeat]──────────────────▶ message_bus (inbound)
                                              │
                                      agent_loop.c  (Core 1)
                                      LLM + tool calls (up to 4 per turn)
                                              │
...
| Core | Tasks |
|------|-------|
| 0 | WiFi/lwIP, HTTP+WS server (port 80), outbound dispatch, Telegram polling, serial CLI, UAC PTT task |
...
4. Agent replies → text sent to browser immediately via `{"type":"message","content":"..."}`
5. `tts_generate()` POSTs to Groq PlayAI or local mlx-audio (5–40s) → WAV cached in PSRAM
6. `{"type":"tts_ready","tts_id":"<8hex>"}` follow-up → browser attaches `▶ Listen` button → fetches `GET /tts/<id>`
...
### 2026-04-07 — Tool call cap raised, STT stack monitoring, TTS async delivery

- **`LANG_MAX_TOOL_CALLS` 2 → 4** — The LLM can now request up to 4 tools in a single turn. The SRAM guard in `agent_loop.c` prevents parallel overrun (89 KB free isn't enough for 3+ parallel 16 KB tasks); sequential execution of 4 tools is safe.
- **STT stack watermark logging** (`main/audio/audio_pipeline.c`) — Logs `stt_task stack HWM: N bytes free` after every transcription. Target: ≥4 KB on 24 KB PSRAM stack. If below threshold, raise to 26 KB.
- **TTS async two-phase delivery** (`main/agent/agent_loop.c`, `main/gateway/ws_server.c`) — Browser receives `{"type":"message","content":"..."}` immediately when LLM finishes (no blocking on TTS generation). A follow-up `{"type":"tts_ready","tts_id":"<id>"}` arrives 2–40 s later; browser attaches `▶ Listen` button to the last assistant bubble.
