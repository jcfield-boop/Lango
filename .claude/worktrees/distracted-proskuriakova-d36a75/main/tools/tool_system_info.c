...
esp_err_t tool_system_info_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    int lfs_pct = lfs_total ? (int)(lfs_used * 100 / lfs_total) : 0;
    ...
    ESP_LOGI(TAG, "system_info: heap=%lu min=%lu psram=%lu sram=%lu lfs=%u/%u uptime=%s rssi=%d",
             (unsigned long)free_heap, (unsigned long)min_heap,
             (unsigned long)free_psram, (unsigned long)free_sram,
             (unsigned)lfs_used, (unsigned)lfs_total,
             uptime_str, rssi);
