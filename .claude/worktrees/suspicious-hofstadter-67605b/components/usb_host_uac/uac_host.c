...
static esp_err_t uac_control_transfer(uac_device_t *uac_device, int len, uint32_t timeout_ms)
{
    ...
    /* PATCH (Langoustine): HS-only devices on FS USB (ESP32 OTG) can report
     * actual_num_bytes > num_bytes on OUT control transfers because the HAL
     * counts STATUS-phase bytes differently.  The transfer completed (checked
     * above) so the byte-count mismatch is a false-positive for OUT requests.
     * For IN transfers we still enforce the check to catch buffer overflows. */
    {
        const usb_setup_packet_t *_sp = (const usb_setup_packet_t *)ctrl_xfer->data_buffer;
        if (_sp->bmRequestType & USB_BM_REQUEST_TYPE_DIR_IN) {
            UAC_RETURN_ON_FALSE(ctrl_xfer->actual_num_bytes <= ctrl_xfer->num_bytes,
                                ESP_ERR_INVALID_RESPONSE, "Incorrect number of bytes transferred");
        } else if (ctrl_xfer->actual_num_bytes > ctrl_xfer->num_bytes) {
            ESP_LOGW(TAG, "OUT ctrl xfer actual=%d > expected=%d — FS-USB quirk, ignoring",
