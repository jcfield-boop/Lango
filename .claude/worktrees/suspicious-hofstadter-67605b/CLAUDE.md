...
**EAGLE voice router** (`agent_loop.c` Slice 3, voice channel only, gated behind `voice_router` NVS flag / CLI kill switch). Before the normal Apfel→Ollama→cloud tier chain runs on a `chat_id=="ptt"` turn, one Apfel call classifies the query into `DIRECT | TOOLS | RACE` via `llm_apfel_router_call()` with a 3000 ms hard deadline (router prompt at `/lfs/config/voice_router.md`, parsed at `context_build_voice_router_prompt()`). Three paths:
- **DIRECT (clean)** — router answer becomes `final_text`, the ReAct loop is skipped entirely. "hi" completes in ~1.2 s vs. ~4-5 s on the legacy cloud path.
- **TOOLS / RACE** — router returns an ack ("Checking the weather…"). The ack is TTS'd and starts playing via `i2s_audio_play_wav_async()` immediately; the legacy tier chain then runs cloud + tools to produce the full answer, which is enqueued as seg B via the existing streaming-TTS pre-population trick (router stashes its `tts_id` into `stream_ctx.tts_job`). User hears ack within ~1.8 s; the real answer replaces silence-before-cloud. Hedge detection (`LANG_VOICE_ROUTER_HEDGE_PATTERNS` + `LANG_VOICE_ROUTER_TIMELY_TOKENS`, both pipe-separated in `langoustine_config.h`) rescues misclassified turns: if a DIRECT answer contains "i'm not sure / might be / currently / right now / …", it's promoted to RACE so the cloud can supersede the (likely hallucinated) router guess. "today" was deliberately dropped from the timely-tokens list — Apfel uses it in polite greetings and it was tripping every DIRECT-clean turn.

Kill switch: `voice_router on|off|status|test <query>` in the serial CLI, persisted under `LANG_NVS_LLM:voice_router`. Default off; turn on after Slice 3 soak with `voice_router on`. OLED rotate-line slot 2 shows the routing decision (`v:D 1160ms`, `v:T 3072ms+cloud`, `v:R 3072ms+cloud`, or `v:- <reason>` for fall-through). Any router error (Apfel offline, timeout, parse fail, OOM) transparently falls through to the legacy tier chain.
