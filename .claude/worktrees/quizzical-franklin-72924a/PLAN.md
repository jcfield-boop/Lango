...
### 1. ~~Drop TLS → plain HTTP/WS~~ ✅ Done (e7b1847)

SRAM free at idle rose to ~47–57 KB as expected.

### 2. Fix httpd deadlock — "never processing requests" ✅ Done (this commit)

See "Already fixed" section for full root-cause analysis and fix summary.
...
- **httpd deadlock / "never processing requests"** — `ws_handler` was calling
  `httpd_ws_send_frame_async` (via `ws_server_send_error` / `send_json_to_fd`)
  from within the httpd task. When Core 1 flooded async sends during LLM
  streaming, the ctrl socket buffer filled, `httpd_queue_work` blocked holding
  `s_send_lock`, and the httpd task blocked trying to acquire `s_send_lock` —
  deadlock. Fix: added `ws_send_error_sync` / `ws_send_json_sync` helpers that
  use `httpd_ws_send_frame(req, …)` (synchronous, no queuing) for the four
  in-handler send sites in `ws_handler`. Removed the spurious
  `ws_server_send_error` from `audio_ring_append` overflow path (caller now
  checks return value). Reduced `recv_wait_timeout` 120 → 30 s (10 s margin
  over 20 s ping interval). Fixed `mcp_post_handler` / `handle_tools_call` to
  run tools in a 16 KB background task with 10 s timeout so slow tool calls
  can't block the httpd task indefinitely.
- Drop TLS → plain HTTP/WS on port 80 (e7b1847)
