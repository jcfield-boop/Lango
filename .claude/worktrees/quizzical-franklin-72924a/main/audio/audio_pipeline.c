...
esp_err_t audio_ring_append(const uint8_t *data, size_t len)
{
    ...
    /* Overflow guard — return ESP_ERR_NO_MEM; caller (ws_handler) sends the
     * error frame via ws_send_error_sync() to avoid the async-from-httpd-task
     * deadlock (httpd_ws_send_frame_async → blocked ctrl socket → s_send_lock
     * deadlock with Core 1 streaming tokens). */
