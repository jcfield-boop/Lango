...
### 2026-05-07 — Fix httpd deadlock ("never processing requests")

**Root cause** (`main/gateway/ws_server.c`, `main/audio/audio_pipeline.c`, `main/mcp/mcp_server.c`):
The httpd task was deadlocking with Core 1 (agent loop streaming tokens). When Core 1 flooded
`httpd_ws_send_frame_async` calls, the internal ctrl socket buffer filled and
`httpd_queue_work()` blocked holding `s_send_lock`. The httpd task, inside `ws_handler`,
tried to send an error frame via `ws_server_send_error` → `send_frame_locked` → blocked on
`s_send_lock`. Classic two-task deadlock; lwIP kept accepting TCP connections but the httpd
task never returned to service them.

**Fixes:**
- Added `ws_send_error_sync()` / `ws_send_json_sync()` helpers that use the synchronous
  `httpd_ws_send_frame(req, …)` API (no queuing, no ctrl socket) for sends that occur
  within the httpd task's own request handlers. Replaced all four async-from-handler
  send sites in `ws_handler` (frame too large, rate limit, audio ring busy, app-layer pong).
- Removed spurious `ws_server_send_error()` call from `audio_ring_append()` overflow path
  in `audio_pipeline.c`; ws_handler now checks the return value and sends the error sync.
- `mcp_post_handler` / `handle_tools_call`: tool execution now runs in a 16 KB background
  task (like agent's `tool_par`). httpd task waits at most `MCP_TOOL_TIMEOUT_MS` (10 s) —
  prevents slow tools (web_search, weather) from starving all other HTTP requests.
- `cfg.recv_wait_timeout` reduced 120 → 30 s (10 s above the 20 s WS ping interval).
...
- **INMP441 mic finally delivers clean audio** (`main/audio/i2s_audio.c`) — Root cause for mic peaks permanently railing at ±32000 in a silent room: `i2s_audio_read()` disables and re-enables the slave RX channel on every call (the ESP32-S3 DMA-stall workaround), and the first DMA word coming back is transient junk from the subsystem stabilising (observed pattern: word[0]=`0xC0000000` followed by healthy samples decoding to ±5 int16). WakeNet was seeing a permanent 32k spike at the head of every buffer, poisoning peak stats and forcing the threshold sky-high. Fix: skip the first 16 DMA words per read (1 ms @ 16 kHz, negligible). Ambient peaks now read ±400 to ±1200 as expected.
- **Relaxed wake-word gain clamp** (`main/audio/wake_word.c`) — Previous NVS sanity clamp had a floor of `WW_DEFAULT_GAIN`, which silently upgraded any sub-default value back to default — impossible to tune gain down for hot mics. New range is `(0.1, 50.0)` to match the runtime setter. Same booby-trap removed earlier from the threshold clamp.
- **Smart routing catches current-events queries** (`main/agent/agent_loop.c`) — `needs_tools` heuristic expanded with temporal markers (`today`, `tonight`, `this week`, `weekend`, `latest`, `current`, `right now`) and current-events phrasing (`what's on`, `happening`, `events`, `concerts`, `news`, `score`, `price of`, `near me`). Previously "what's on in SF" routed to Apfel (which has no tools) and came back with a refusal. Now it escalates straight to Ollama / cloud with `web_search` available.
- **Gated auto-email on LLM tier + raised threshold** (`main/agent/agent_loop.c`) — Auto-email now requires `!using_apfel` and response length > 600 chars (was > 200). Prevents Apfel refusals from triggering spurious briefing emails.
- **Beginner-focused surf reports** (`littlefs_data/config/USER.md`, `littlefs_data/HEARTBEAT.md`) — USER.md now records the user's surf skill level and safe wave-height range (1–3 ft comfortable, 4 ft upper limit, 5 ft+ unsafe). Morning briefing and Saturday 18:00 forecast rewritten with a 5-tier ⭐ beginner rubric, an honest GO / MAYBE / SKIP verdict, and a cross-check against Surfline because NOAA buoy 46012 overreads Linda Mar (beach is 40–60 % of the open-ocean reading due to the Pillar Point lee).
- **Back to `openrouter/auto` as cloud default** (`main/langoustine_config.h`) — Reverted build-time default from `openai/gpt-4o-mini` so fresh flashes get OpenRouter's auto-router out of the box.

### 2026-04-08 — Apfel: Apple Foundation Model as fast local tier

- **Three-tier LLM routing** (`main/llm/llm_proxy.c`, `main/agent/agent_loop.c`, `main/agent/context_builder.c`) — Added [Apfel](https://apfel.franzai.com) as a third routing tier alongside Ollama and cloud. Apfel exposes Apple's on-device Foundation Model (~3B params, runs on the Neural Engine, near-zero RAM, ~1 s response) via an OpenAI-compatible `/v1/chat/completions` API. Used for simple conversational queries where no tools are needed.
- **Minimal context builder for Apfel's 4K window** (`main/agent/context_builder.c`) — New `context_build_minimal_prompt()` targets ~400 tokens: assistant identity, timezone, current time. No SOUL/MEMORY/skills/tool schemas. Leaves ~3600 tokens for history + user message + response within Apfel's hard 4096-token context window.
- **Unified hierarchy for voice + text + Telegram** — Same routing rules apply everywhere: simple queries → Apfel → Ollama → cloud; tool-triggering queries → Ollama → cloud; complex/system queries → cloud directly. Voice queries (`chat_id="ptt"`) still add `max_tokens=400` and the VOICE MODE prompt injection.
- **SERVICES.md Apfel section** (`main/config/services_config.c`) — New `## Apfel (Apple Foundation Model)` section with `base_url:` and `model:` keys. Hot-reloadable like all other service config.
- **OLED "A+ / A-" indicator** (`main/display/oled_display.c`) — Idle screen now shows Apfel status alongside Ollama (`O`) and mlx-audio (`F`).
